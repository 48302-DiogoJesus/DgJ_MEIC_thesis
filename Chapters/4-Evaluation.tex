...
\begin{figure}[H]
    ...
    \caption{\textit{Makespan} vs Resource Usage comparison to WUKONG}
    \label{fig:makespan_resource_usage_tradeoff}
\end{figure}

Chart presented in Figure~\ref{fig:makespan_resource_usage_tradeoff} isolates the impact of the three baseline (non-optimized) algorithms, revealing a clear trade-off between execution speed and resource efficiency. A direct comparison between our planners shows that the \textbf{NonUniformPlanner} achieves the fastest makespan at 9.25s. However, this speed is costly; the \textbf{UniformPlanner}, while approximately 10\% slower with a makespan of 10.14s, presents a vastly more \textit{resource-frugal} profile. It consumes 18\% less CPU-seconds and an exceptional 290\% less GB-seconds than the NonUniformPlanner. When compared to the \textbf{WUKONGPlanner} baseline, our planners show significant advantages. WUKONG is the slowest by a considerable margin, with a makespan of 11.92s---nearly 29\% slower than the NonUniformPlanner. Furthermore, it is the most CPU-intensive, demanding approximately 69\% more CPU-seconds than the UniformPlanner. While WUKONG's memory usage is more moderate than that of the NonUniformPlanner, it is still over 108\% higher than the lean UniformPlanner, establishing the latter as the superior choice for balanced performance.
