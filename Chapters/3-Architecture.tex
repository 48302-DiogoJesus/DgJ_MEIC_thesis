...
Since our target execution platform is FaaS, the worker logic is implemented as a FaaS handler. Due to the decentralized nature of our solution, workers will be responsible for performing both task execution and scheduling in a choreographed manner. The worker loop starts when the worker is invoked by the FaaS gateway. When invoked, a worker receives a \textbf{configuration}, with information required to connect to storage and the FaaS gateway, the user SLA, and other data; the \textbf{workflow instance identifier}; a list of \textbf{task identifiers} which indicate the branches the worker should handle, executing tasks assigned to it and delegating the others to the appropriate workers. % TODO: continue
