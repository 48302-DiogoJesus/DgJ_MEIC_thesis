...
After a plan is created and validated, the workflow representation is serialized using \textit{cloudpickle} (as mentioned before) and uploaded to storage. The plan is embedded directly within this representation, allowing each worker to identify both the tasks it is responsible for executing and the ones it must delegate to other workers. In the following section, we describe in detail how workers operate, including how they follow the plan, handle task dependencies, and coordinate through event-based communication to achieve decentralized workflow execution.

% #############################################################################

\section{Client}
\label{s:client}
TODO: otimizar DAG structure para ficar mais pequeno\\
TODO: hardcoded inputs: grandes e pequenos\\
TODO: executar planner\\
TODO: esperar pela notificação que está completo + fazer download do resultado\\
TODO: gerar uma imagem do workflow sem o executar (semelhante ao Dask e WUKONG)\\
TODO: gerar uma imagem do plano também\\
TODO: submeter o worklfow (compute())\\
TODO: um dashboard com uma visualização simples do DAG, onde dá para perceber a progressão do workflow\\
TODO: transição p/ workers section\\
