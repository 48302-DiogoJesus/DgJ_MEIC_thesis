...
\begin{lstlisting}[language=Python, caption={Overridable worker execution logic stages}, label=lst:optimization_methods]
    ...
    async def wel_override_should_upload_output(planner, storage, dag, this_worker, current_task) -> bool | None
    
    async def wel_update_dependency_counters(planner, storage, dag, this_worker, current_task) -> list | None

    async def wel_override_handle_downstream(planner, storage, dag, this_worker, current_task, downstream_tasks_ready) -> list | None
\end{lstlisting}
...
Each branch of task handling runs concurrently in its own coroutine, following these sequential phases:
\begin{enumerate}
