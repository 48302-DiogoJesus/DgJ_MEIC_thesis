...
After filtering samples, we use an algorithm (Algorithm \ref{alg:select_related_samples_percentile}) that selects a limited number of the most relevant samples for each prediction. Given a reference input, the method computes a baseline from all available samples, either as the average or a user-specified percentile depending on the user-chosen SLA. It then iteratively considers increasing thresholds of similarity/proximity to the value to predict, starting from 5\% of the baseline up to 100\%, and collects samples whose associated input values fall within each threshold. Priority is given to samples that are closest to the reference value, with a balance between samples slightly below and slightly above the reference. If the number of samples meeting the threshold is sufficient, the method selects up to a maximum number of samples, favoring those nearest to the reference. If no threshold yields enough samples, it falls back to selecting the closest available samples. 
