...
The \textsc{AssignGroup} function, presented in Algorithm~\ref{alg:assign_group}, is responsible for clustering tasks into execution groups and assigning them to workers. After retrieving the predicted execution times and output sizes for each task (lines 4-5), the algorithm classifies tasks as either \textit{long} or \textit{short} depending on whether their predicted execution time is above or below the median (lines 7-8). This categorization allows the planner to balance worker load.

The function proceeds in four stages:
\begin{enumerate}
    \item \textbf{Local clustering (lines 10-14):} If an upstream worker is available, a subset of \textit{short} tasks with larger output sizes (up to a predefined maximum cluster size) is assigned to the same worker. This promotes \textit{data locality}, reducing data transfer costs by executing tasks near their data sources;
    
    \item \textbf{Balanced pairing (lines 16-22):} The algorithm pairs each remaining \textit{long} task with a few \textit{short} ones, creating mixed groups, distributing workload more evenly;
    
    \item \textbf{Grouping remaining short tasks (lines 24-28):} Any remaining unassigned \textit{short} tasks are grouped together and allocated to new workers, up to the maximum cluster size;
    
    \item \textbf{Grouping remaining long tasks (lines 30-35):} This final phase is only reached when there are more long tasks than short ones, meaning some long tasks could not be paired in the previous phase. In such cases, the remaining long tasks are grouped into smaller clusters (half the normal cluster size) to try reducing resource contention.
\end{enumerate}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth, keepaspectratio]{images/mine/text_analysis_NonUniformPlanner-opt.png}
